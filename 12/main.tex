...
% PGFがある特定の階層を定義
\newcommand{\pgfpath}{\assetpath/1020201.アルゴリズム２/12/images/}
...
\textbf{次の目標（現場の目標）}
\begin{itemize}
  \item ポテトは\textbf{利益率が高い}。
  \item だから店長は \textbf{ポテトの売上を伸ばす}ことを目標にしました。
\end{itemize}
...
%   page 03
% ----------------------------------------------------------------------------------------
\begin{frame}{前回の検定は「判断」／今回の回帰は「予測」}
\small
...
\vspace{0em}
\emjdotblackS \markline{やること（操作は実習①と同じ）}
\vspace{-0.8em}
\begin{enumerate}
  \setlength{\itemsep}{0mm} % ここで間隔をゼロ（最小）にする
...
\vspace{-0.5em}
\emjdotblackS  \uline{\textbf{比較するポイント（ここが学び）}}
\vspace{-0.6em}
  \begin{itemize}
...
\vspace{-0.6em}
\emjdotblackS \textbf{理由づけ：メモ列で説明する}\\
