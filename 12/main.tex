...
\graphicspath{{images/}{\assetpath/1020201.アルゴリズム２/12/images/}{../project_assets/images/}{../project_assets/emoji/emoji_pngs/}}

%--------------------------
% フッター
%--------------------------
\newcommand{\myfootertext}{1020201.アルゴリズム２/12}
...
\date{}
\newcommand{\codedir}{\assetpath/1020201.アルゴリズム２/12}
...
\begin{frame}{枠線付きブロックのテスト}
    % グリーンベースの配色定義
    \definecolor{MyDarkGreen}{HTML}{2E7D32}  % タイトル背景
    \definecolor{MyLightGreen}{HTML}{E8F5E9} % 本文背景
    \definecolor{MyDarkGreen}{HTML}{587a7f}

    \begin{tcolorbox}[
%        colback=MyLightGreen,    % 本文の背景色
        colback=white,    % 本文の背景色
        colframe=MyDarkGreen,    % 枠線の色
        colbacktitle=MyDarkGreen, % タイトルの背景色
        coltitle=white,          % タイトルの文字色
        fonttitle=\bfseries,     % タイトルを太字に
        title={正規分布の定義},    % タイトルテキスト
        sharp corners=south,     % 下側の角だけ尖らせる（Canva風アレンジ）
        boxrule=1pt              % 枠線の太さ
    ]
        ここが本文エリアです。外側にタイトルと同じ色の枠線が入ることで、
        内容が引き締まって見えます。
    \end{tcolorbox}
\end{frame}

\begin{frame}{tcolorboxで作るCanva風デザイン}

    % --- 色の定義（落ち着いたグリーンベース） ---
    \definecolor{CanvaGreen}{HTML}{2E7D32} % タイトル＆枠線の色
    \definecolor{PaleGreen}{HTML}{F1F8E9}  % 本文の背景色

    \begin{tcolorbox}[
        enhanced,                          % 高度な設定を有効化
        title={実習の重要ポイント},            % タイトル文字列
        colback=PaleGreen,                 % 本文の背景
        colframe=CanvaGreen,               % 枠線の色（タイトル背景と同じ）
        coltitle=white,                    % タイトル文字の色
        fonttitle=\bfseries\sffamily,      % タイトルを太字・ゴシック体に
        sharp corners=northwest,           % 左上の角だけ尖らせる（Canva風の遊び心）
        arc=3mm,                           % 角の丸み
        boxrule=1.5pt,                     % 枠線の太さ
        left=4mm, right=4mm, top=3mm,      % 内側の余白調整
        toptitle=2mm, bottomtitle=2mm      % タイトル内の余白
    ]
        本文のエリアに枠線を入れることで、情報の「まとまり」が強調されます。
        TikZで描いたグラフなどをこの中に入れることも可能です。
        
        \begin{itemize}
            \item 色の統一感（タイトル背景 ＝ 枠線）
            \item 適度な余白（Padding）
            \item フラットな質感（影なし）
        \end{itemize}
    \end{tcolorbox}
