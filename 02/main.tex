...
\graphicspath{{images/}{\assetpath/1020201.アルゴリズム２/02/images/}{../project_assets/images/}{../project_assets/emoji/emoji_pngs/}}
% PGFがある特定の階層を定義
\newcommand{\pgfpath}{\assetpath/1020201.アルゴリズム２/02/images/}

%--------------------------
% フッター
%--------------------------
\newcommand{\myfootertext}{1020201.アルゴリズム２/02}
...
\title{ 02 統計で何ができるか }
\date{}
\newcommand{\codedir}{\assetpath/1020201.アルゴリズム２/02}
...
今日から、\textbf{\ruby{統計}{とうけい}}の学習を始めます。
...
\begin{itemize}
  \item \textbf{\ruby{要約}{ようやく}：} たくさんのデータを、平均値などの「1つの数字」で表現する。
  \item \textbf{\ruby{判断}{はんだん}：} 新しい施策に効果があったのか、偶然なのかを見極める。
  \item \textbf{\ruby{予測}{よそく}：} 過去の傾向から、将来のリスクや可能性を予見する。
\end{itemize}
...
\begin{itemize}
  \item \textbf{\ruby{公表値}{こうひょうち}：} ポテトMサイズは \textbf{135g} とされている
  \item \textbf{現場：} すべてを毎回計量するのは難しく、\textbf{\ruby{目分量}{めぶんりょう}}になりやすい
...
「私」は噂を確かめるため、実際に1週間毎日ポテトを購入して測ってみることにしました。

\begin{columns}
    \begin{column}{0.55\textwidth} % 少し幅を調整
      \vspace{0pt}
        % 高さを明示的に制限する（例：画面の40%まで）
        \includegraphics[width=\textwidth, height=0.6\textheight, keepaspectratio]{秤に乗ったポテトの写真.png}
    \end{column}
    \begin{column}{0.43\textwidth}
      \vspace{0pt}
        \textbf{【調査ルール】}
        \begin{itemize}
            \setlength{\itemsep}{0.5ex} % 箇条書きの間隔を狭くする
            \item[①] 毎日Mサイズを1袋買う
            \item[②] スケールで重さを測る
            \item[③] 味や調理状態も記録
        \end{itemize}
...
%   page 08
% ----------------------------------------------------------------------------------------
\begin{frame}{【結果】どのぐらいか判断できるのか？}
1週間分のデータを集計すると、次のようになりました。

\begin{center}
    \begin{tabular}{|c|c|c|c|c|c|c|c|}
    \hline
    曜日 & 月 & 火 & 水 & 木 & 金 & 土 & 日 \\ \hline
    重さ & 130g & 138g & 125g & 140g & 132g & 129g & 137g  \\ \hline
    \end{tabular}
\end{center}

\begin{itemize}
    \item 135gを超える日もあるが、低い日もある
    \item 公式値より 大きいか小さいか
\end{itemize}
...
しかし——
\begin{itemize}
    \item 公表値より大きい、小さいが問題と言えるのか？
    \item それとも、\textbf{たまたま}起きたものなのか？
\end{itemize}
...
これまではSNSの評判や個人の感想といった「\ruby{主観的}{しゅかんてき}な情報」を扱ってきました。
ここからは、「私」が実際に店舗へ足を運んで集めた「データ」を使って、\ruby{客観的}{きゃっかんてき}に事実を確かめていきます。
...
コンピューター（Excel）だけを使うのではなく、自分でデータを眺めて実感することが大切です。
...
「平均」は、バラバラな重さのデータを「もし全部同じだったら？」と平らにならしたときの値です。\par
\textbf{平均の計算：}
\begin{columns}
    \begin{column}{0.45\textwidth} % 左側60%の幅を使用
...
\begin{itemize}
  ...
  \item \textbf{操作手順：}
  \begin{enumerate}
    \item 答えを表示したいセルを選択する。(C9)
    \item \texttt{=AVERAGE(} と入力する。
    \item 「私」のデータの「重さ\_g（7日分）」をマウスで選択する。(=AVERAGE(C2:C8))
    \item Enterキーを押す。
  \end{enumerate}
\end{itemize}

\textbf{今回の「私」の結果：}\\
計算すると $\mathbf{133.0}$g になりましたか？目標の $135$ g に近いでしょうか。
...
平均（中心）だけでなく、「データの\ruby{端}{はし}」を見ることも非常に重要です。
...
\textbf{今回の「私」の結果：}\\
最大は $\mathbf{140.0}$g、最小は $\mathbf{125.0}$g になりましたか？
...
\textbf{今回の「私」の結果：}\\
中央値は $\mathbf{132.0}$g になりましたか？ 平均値（$133.0$）と少しズレがあることがわかります。
