...
\graphicspath{{images/}{\assetpath/1020201.アルゴリズム２/11/images/}{../project_assets/images/}{../project_assets/emoji/emoji_pngs/}}
% PGFがある特定の階層を定義
\newcommand{\pgfpath}{\assetpath/1020201.アルゴリズム２/11/images/}

%--------------------------
% フッター
%--------------------------
\newcommand{\myfootertext}{1020201.アルゴリズム２/11}
...
\title{ 11 仮説検定②（平均の検定） }
\date{}
\newcommand{\codedir}{\assetpath/1020201.アルゴリズム２/11}
...
\begin{tcolorbox}[title={今日やること（結論）}]
ポテトのデータを使って、\\
...
\vspace{0.3em}
\begin{tcolorbox}[title={今日の問い（平均の判断）}]
...
\vspace{0.6em}
\begin{tcolorbox}[title={判断の型（必ずこの順番）}]
\begin{enumerate}
...
\vspace{-0.5em}
\begin{tcolorbox}[title={考え方（ここが検定の核）}]
...
\vspace{0.6em}
\begin{tcolorbox}[title={母分散（$\sigma^2$）・母標準偏差（$\sigma$）}]
\begin{itemize}
...
\vspace{-0.3em}
\begin{tcolorbox}[title={ここが重要：平均のゆれ（標準誤差）が固定できる}]
...
\small
\vspace{-2.5ex} % タイトル下の隙間をさらに削る

前回は、帰無仮説 $H_0:\mu=\mu_0$ のもとで、\\
標本平均 $\bar{x}$ がどれくらい珍しい位置にあるかを、\textbf{Z値}で表しました。

\begin{tcolorbox}[title={Z値（位置）の形}, top=0mm, bottom=0mm, before=\vspace{0.2ex}, after=\vspace{0.2ex}]
\[
...
\begin{itemize}
  \setlength{\itemsep}{0mm} % 箇条書きの間隔をゼロに
  \item 分子 $\bar{x}-\mu_0$：\textbf{ずれ}（平均との差）
  \item 分母 $\sigma/\sqrt{n}$：\textbf{平均のゆれ}（標準誤差）
  \item つまり $Z$ は、\textbf{ずれが「ゆれ」の何倍か}（位置）
\end{itemize}

\vspace{-1.5ex} % カラム前の調整
\begin{columns}[t]
...
\centering
\vspace{-12.0ex} % ★ここ！ 負の値を大きくしてグラフを上に引き上げる
\resizebox{1.0\linewidth}{!}{\input{\pgfpath z_distribution_plot.pgf}}
\end{column}
\end{columns}

\vfill
\emjpin \textbf{この「位置」$\rightarrow$ p値 $\rightarrow$ 判断の流れは今日も同じ}
...
\vspace{-0.5em}
\begin{tcolorbox}[title={ここで問題になる点}]
...
\vspace{0.6em}
\begin{tcolorbox}[title={標本標準偏差 $s$}]
...
\vspace{0.6em}
\begin{tcolorbox}[title={平均のゆれ（標準誤差）も、標本から作る}]
\[
...
\vspace{-0.6em}
\begin{tcolorbox}[title={Excelで使う（これでOK）}]
\begin{itemize}
...
\begin{tcolorbox}[title={今日はここだけ覚える（ルール）}]
\begin{itemize}
...
\vspace{0.6em}
\begin{tcolorbox}[title={結論（ここが「なぜt？」の答え）}]
...
\end{itemize}
\vspace{-1ex} % カラム前の隙間を調整
\begin{columns}[t]
\begin{column}{0.58\linewidth}

\begin{tcolorbox}[title={Zとの違い（ここだけ）}]
\begin{itemize}
  \setlength{\itemsep}{0mm}
...
\vspace{-12.0ex} % ★ここ！ 負の値を大きくしてグラフを上に引き上げる
\resizebox{1.2\linewidth}{!}{\input{\pgfpath distribution_plot1.pgf}}

\vspace{-2.0ex} % ★グラフ下の余白も削る
\end{column}
\end{columns}

\vfill % 余ったスペースを自動で割り振る
...
\vspace{0.6em}
\begin{tcolorbox}[title={p値（両側）の意味（文章）}]
...
\vspace{-0.4em}
\begin{tcolorbox}[title={Excel（例：両側）}]
...
\begin{column}{0.40\linewidth}
\centering
\vspace{-1.0em} % グラフを上に少し引き上げ
\resizebox{1.2\linewidth}{!}{\input{\pgfpath p_value_plot.pgf}}

\vspace{-2.0em} % グラフ下の余白を削る
\scriptsize
両側：左右の外側を合計
\end{column}
\end{columns}
...
\vspace{0.6em}
\begin{tcolorbox}[title={結論文テンプレ（t検定）}]
...
\vspace{0.6em}
\begin{tcolorbox}[title={この210件でやりたいこと（推測）}]
\begin{itemize}
...
\vspace{0.6em}
\begin{tcolorbox}[title={今回の実習の流れ（推測→判断）}]
\begin{enumerate}
...
\vspace{0.6em}
\begin{tcolorbox}[title={5ステップ（この順番で作業する）}]
\begin{enumerate}
...
\vspace{0.6em}
\begin{tcolorbox}[title={例（対応ありの状況）}]
同じ人のポテトを「\textbf{月曜}」と「\textbf{金曜}」で比べる。\\
（人による食べ方・取り分けのクセを打ち消したい）
\end{tcolorbox}

\vspace{0.5em}
\begin{tcolorbox}[title={考え方（ここだけ）}]
\begin{enumerate}
...
\vspace{0.6em}
\begin{tcolorbox}[title={今日できるようになったこと}]
\begin{itemize}
